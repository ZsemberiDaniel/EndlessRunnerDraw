...
public static class Math3D {
    ...
    public static bool IsPointOnLine(Vector3 linePoint1, Vector3 linePoint2, Vector3 point) {
        return Mathf.Approximately(Vector3.Distance(linePoint1, point) + Vector3.Distance(linePoint2, point), Vector3.Distance(linePoint1, linePoint2));
    }


    public static bool ArePointsOnLine(Vector3 linePoint1, Vector3 linePoint2, params Vector3[] points) {
        float lineDist = Vector3.Distance(linePoint1, linePoint2);

        for (int i = 0; i < points.Length; i++) {
            if (!Mathf.Approximately(Vector3.Distance(linePoint1, points[i]) + Vector3.Distance(linePoint2, points[i]), lineDist))
                    return false;
        }
